...
## Agency Signals in Conversations

While topic clustering focuses on *what* is being discussed, agency analysis captures *how* participants frame their contributions. We score every translated message with the BERTAgent model, which was trained to output positive values for proactive, agentic statements and negative values for passive or low-agency language. Unlike topic modeling, no short-message filtering is applied—BERTAgent handles brief acknowledgements by returning scores near zero.
...
```{python}
#| label: tbl-agency-examples
if not agency_messages.empty:
    def role_examples(role: str, label: str) -> None:
        subset = agency_messages[agency_messages["message_role"] == role].copy()
        subset = subset.dropna(subset=["agency_score"])
        if subset.empty:
            print(f"No scored {label.lower()} messages available.")
            return
        ...
        if not top_pos.empty:
            print(f"Top high-agency {label} messages")
            display(top_pos.reset_index(drop=True))
        if not top_neg.empty:
            print(f"Lowest agency {label} messages")
            display(top_neg.reset_index(drop=True))

    role_examples("user", "User")
    role_examples("assistant", "Assistant")
else:
    print("No agency scores available for qualitative review.")
```
...
**Prioritize High-Impact Regions.** Since the top regions account for the majority of student interactions, development resources should be allocated to enhance content, AI responses, and user experience tailored to these high-volume geographic contexts. This focused investment will directly address where students spend most of their time and where improvements will have the greatest impact.
