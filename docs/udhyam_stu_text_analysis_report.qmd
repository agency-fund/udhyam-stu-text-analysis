...
Every message in the dataset is scored using the BERTAgent model, a machine learning classifier specifically trained to recognize and quantify agency signals in text:

1. **Model training**: BERTAgent was developed by fine-tuning a transformer-based language model on a corpus of labeled conversational data where messages were annotated for agency levels. The model learned to recognize linguistic patterns, syntactic structures, and semantic cues that correlate with agentic versus passive communication.

2. **Score generation**: For each message, BERTAgent produces a continuous numerical score reflecting the degree of agency expressed. Positive scores indicate proactive, agentic language while negative scores indicate passive or low-agency communication. Scores near zero reflect neutral acknowledgments or brief responses that carry minimal agency information.

3. **Threshold-based categorization**: To enable summary statistics, scores are classified into three categories using established thresholds: high-agency messages (score ≥ 0.3), neutral-agency messages (score between -0.3 and 0.3), and low-agency messages (score ≤ -0.3).

4. **Message handling**: Unlike topic modeling which filters short messages, BERTAgent is designed to handle messages of all lengths, including very brief acknowledgments. The model naturally returns near-zero scores for these brief exchanges, which accurately reflect their minimal agency content.

5. **Role-based analysis**: Agency scores are analyzed separately for user and assistant messages to understand how each participant's communication patterns contribute to the overall conversation dynamics.

This approach provides nuanced insights into the motivational and empowerment dynamics of student-AI interactions, complementing the content-focused analysis provided by sentiment and topic modeling.
